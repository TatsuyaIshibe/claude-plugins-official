...
function echoKey(raw: string): string {
  // Normalize aggressively for echo detection:
  // - Strip signature before other transforms
  // - Remove ZWJ and emoji variation selectors (chat.db inconsistent)
  // - Normalize smart quotes/dashes
  // - Remove all whitespace variants
  // - Lowercase and cap length
  return raw
    .replace(/\s*Sent by Claude\s*$/, '')           // strip signature first
    .replace(/[\u200d\ufe00-\ufe0f]/g, '')          // ZWJ + variation selectors
    .toLowerCase()
